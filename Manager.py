...
class UnifiedBlackBotManager:
    ...
    def create_instance_interactive(self) -> bool:
        ...
        try:
            ...
            if ssl_enabled:
                port = 6697
                print_success("SSL enabled - will use secure connection")
...
"""
    ...
    def _create_legacy_settings_file(self, config: BotInstanceConfig):
        """Create instance-specific settings.py for legacy compatibility"""
        try:
            instance_dir = self.instances_dir / config.name
            settings_file = instance_dir / "settings.py"

            # Generate settings.py content based on instance config
            servers_py = repr([f"{s.split(':')[0]} {s.split(':')[1]}" for s in config.servers])
            channels_py = repr(config.channels)

            settings_content = f'''# settings.py for instance: {config.name}
# Auto-generated by Unified BlackBoT Manager
# This provides legacy compatibility for BlackBoT core

# Core Bot Identification
char = "{config.command_char}"
nickname = "{config.nickname}"
username = "{config.username}"
altnick = "{config.nickname}_"
realname = "{config.realname}"
away = "No Away"
...
# SSL Settings
ssl_use = {1 if config.ssl_enabled else 0}
ssl_cert_file = ""
ssl_key_file = ""

# NickServ Login Settings
nickserv_login_enabled = {str(config.nickserv_enabled)}
require_nickserv_ident = True
nickserv_nick = "NickServ"
nickserv_botnick = "{config.nickname}"
nickserv_password = "{config.nickserv_password}"
...
dcc_public_ip = ""
dcc_listen_port = {config.dcc_port}
...
# Logging Configuration
logs_dir = "instances/{config.name}/logs"
...
instance_environment = "{config.environment}"
...
'''
            ...
            print_info(f"Created legacy settings.py for '{config.name}'")

        except Exception as e:
            print_warning(f"Could not create legacy settings: {e}")
