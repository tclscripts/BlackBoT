...
def read_local_version():
    ...
    if path.exists():
        with open(path, "r") as f:
            v = f.read().strip()
            logger.debug(f"Local version found: {v}")
            return v
    logger.debug("No local VERSION file, defaulting to 0.0.0")
    return "0.0.0"
...
def fetch_remote_version():
    ...
    if r.status_code == 200:
        v = r.text.strip()
        logger.debug(f"Remote VERSION = {v}")
        return v
...
def _replace_dir(src_dir: Path, dst_dir: Path):
    if dst_dir.exists():
        logger.debug(f"Removing old directory: {dst_dir}")
        shutil.rmtree(dst_dir)
    logger.debug(f"Copying new directory {src_dir} -> {dst_dir}")
...
def update_from_github(self, feedback):
    ...
    if remote_version == local_version:
        logger.debug(f"Already up to date: {local_version}")
        return

    logger.debug(f"Update available: {remote_version} (current {local_version})")
    ...
    try:
        with tempfile.TemporaryDirectory() as tmpdir_str:
            ...
            logger.debug(f"Downloading archive {zip_url}")
            ...
            logger.debug(f"Extracted to {extracted_dir}, project root is {project_root}")
            ...
            if src_core.exists():
                ...
                logger.debug(f"core/ in archive has {len(py_files)} files")
            ...
            for root, _, files in os.walk(extracted_dir):
                ...
                for file in files:
                    ...
                    if file == CONFIG_FILE:
                        logger.debug("Merging settings.py")
